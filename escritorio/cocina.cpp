void cocina::mostrarDetallePedido(const QJsonObject &pedido)
{
    ...
    if (tieneRepartidor) {
        ui->widget_19->setStyleSheet("background-co#include "cocina.h"
#include "ui_cocina.h"
...
{
    QNetworkRequest request(QUrl("http://localhost/dashboard/APP_ElBuenGusto/escritorio/cocina.php"));
...
void cocina::confirmarPedido()
{
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QJsonObject data;
        data["id_pedido"] = pedidoActual["id"].toInt();  // Usar "id" en lugar de "id_pedido"
